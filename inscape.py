...
# replace post_align (works, I just suspect you'll want to replace with your own aligner)
# improve parsing efficiency (dont parse for each method)
# check input for duplicate sequences (will error out in combineseqs there are any)
# make allcaps variables global? (I had trouble with this so I made them not this way)
# simul_evol is very gross (several reasons)
#post_align needs to be pre_align or smth
#asdfasdf
#fix parallelization
# ~the bad area~
...
def figure(times,predictions,INPUTS,expect):
    ...
    zevo=zscore(times.values())
    zfeatures=zscore(predictions.values())
...
#@profile
def print_components(DSamp,INPUTS,S,C): #complain and print (more than two connected components via QUENTIN metric inside hamming clique) (rare)
    #main
...
##############################################################################################################################################
    ...
    [S,C]=determine_components(DSamp) #S==number of connected components in the graph (number of outbreaks by QUENTIN metric)        
    ...
    if timeSource==featureSource:
        ...
        statement_time="QUENTIN analysis: LOW CONFIDENCE PREDICTION! %s is the predicted source. Analysis took %.3f seconds. Thank you for using QUENTIN!" % (source, workTime)
        if strength==True:
            print('Among the given samples, %s appears to be the most likely candidate for the source' % source)
        statement_time='QUENTIN analyis was successful! Analysis took %.3f seconds. Thank you for using QUENTIN!' % workTime
        ...
        statement_time='QUENTIN analysis: source undeterminable or not present - feature source != evolution source! Analysis took %.3f seconds. Thank you for using QUENTIN!' % workTime
